...
for TBE in TBE_values:
    ...
    plt.plot(data["epsi"], data["TBR_req"], label=f"TBE = {TBE:.1f}%", marker='.')
    plt.sca(axs[1])
    plt.plot(data["epsi"], data["I_startup_kg"], label=f"TBE = {TBE:.1f}%", marker='.')

plt.xlabel("Tritium non-radioactive loss fraction $\Epsilon$ (-)")
...
# save and show
plt.savefig("tbr_startup_inv_vs_epsi.pdf")
plt.show()
